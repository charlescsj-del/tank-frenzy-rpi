...
**Admin:** `/admin` shows every room at once: a live mini-map, phase, mode, time, each player's kills, deaths, health and country, plus server CPU, memory, load and uptime, refreshed twice a second. **Open room** opens its join link; **End room** sends everyone in it back to the room browser. It opens from the **Admin** link when you use the Home Assistant sidebar, or on the public address with the `admin_password` option.
