...
The app has no HA configuration/media mappings. `/data` is reserved for Supervisor options; gameplay state is not persisted. App restarts/updates end active matches.
...
4. Check `http://YOUR_PI_IP:8765/health`; it should return `status: ok` and game version `1.11.0`.
